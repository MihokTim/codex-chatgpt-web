...
test("the existing light slug retains the local Sol Pro route without changing the Pro slug", () => {
  expect(CHATGPT_WEB_LEGACY_MODEL_ROUTES.find(route => route.slug === "chatgpt-web/light")).toMatchObject({
    displayName: "ChatGPT Web — Sol Pro", adapterEffort: "max", codexEffort: "ultra", requiresPro: true,
  });
  expect(CHATGPT_WEB_LEGACY_MODEL_ROUTES.find(route => route.slug === "chatgpt-web/pro")).toMatchObject({
