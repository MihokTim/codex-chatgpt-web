...
const catalogArgument = process.argv.slice(3).find(argument => argument.startsWith("--catalog="));
const sourceCatalog = JSON.parse(catalogArgument
  ? readFileSync(resolve(catalogArgument.slice("--catalog=".length)), "utf8")
  : runCodex(["debug", "models", "--bundled"]).stdout) as { models?: unknown[] };
...
try {
  ...
  if (nativeModel?.multi_agent_version !== "v1" || webPro?.multi_agent_version !== "v1") {
    throw new Error(
      `Codex did not preserve Compatibility V1 catalog metadata: ${JSON.stringify({ nativeModel, webPro })}`,
    );
  }
  ...
  process.stdout.write(`NATIVE_CODEX_CATALOG_SMOKE_OK ${JSON.stringify({ spawnOverrides })}\n`);
} finally {
