...
export const MAX_COMPACTION_HANDOFF_TIMEOUT_MS = 120 * 60_000;
...
export async function requestRetainedCompactionHandoff(
  ...
  try {
    ...
    browser = worker.run({
      ...
      browserEffortOverride: parsed._chatgptModelFamily ? undefined : compactionBrowserEffortOverride(parsed.modelId, parsed.options.reasoning, capabilities),
