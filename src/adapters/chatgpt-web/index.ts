...
export function createChatGptWebAdapter(
  ...
  const startRuntime = (
    ...
    if (hooks.failedThinkingRecovery) trace.push({ kind: "commentary", text: "ChatGPTの応答失敗から復旧します。完了済みのツール結果を保持し、未完了部分を引き継ぎます（1回限り）。" });
    ...
    if (!mode.localTools) {
      const browserTurn = cancellableBrowserTurn(finalizeCheckpoint(worker.run({
        ...
        ...(parsed._compactionRequest ? {
          browserEffortOverride: parsed._chatgptModelFamily ? undefined : compactionBrowserEffortOverride(parsed.modelId, parsed.options.reasoning, turnCapabilities),
        } : {}),
