...
test(`failed fresh-conversation setting restores every mutable setup file before restarting the previous runtime (symlink=${useSymlink})`, async t => {
  ...
  const backingConfigPath = useSymlink ? sharedConfigPath : codexConfigPath;
  fs.writeFileSync(backingConfigPath, "old codex config\n", { mode: 0o640 });
  if (useSymlink) {
    try {
  ...
  const linkTarget = useSymlink ? fs.readlinkSync(codexConfigPath) : undefined;
  const linkInode = fs.lstatSync(codexConfigPath).ino;
  const directoryMode = fs.statSync(sharedDirectory).mode & 0o777;
  const fileMode = fs.statSync(backingConfigPath).mode & 0o777;
  ...
  try {
    ...
    assert.equal(fs.lstatSync(codexConfigPath).isSymbolicLink(), useSymlink);
    if (useSymlink) {
    ...
    assert.equal(fs.statSync(backingConfigPath).mode & 0o777, fileMode);
