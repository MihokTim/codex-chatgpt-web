...
test("fork updates announce upstream releases without overwriting the isolated installation", async () => {
  ...
  try {
    ...
    assert.match((await updater.checkOnce()).message, /v6\.0\.1.*独自修正版/);
    await assert.rejects(updater.beginInstall(), /独自修正版/);
